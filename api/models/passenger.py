...
class PassengerBase(BaseModel):
    ...
    survived: Optional[int] = Field(
        None, ge=0, le=1, description="Überlebensstatus (0=gestorben, 1=überlebt)"
    )
    pclass: Optional[int] = Field(
        None, ge=1, le=3, description="Passagierklasse (1, 2, oder 3)"
    )
    ...
    sibsp: Optional[int] = Field(
        None, ge=0, description="Anzahl Geschwister/Ehepartner an Bord"
    )
    ...
    class_name: Optional[str] = Field(
        None, alias="class", description="Klassename (First, Second, Third)"
    )
...
class PassengerFilter(BaseModel):
    ...
    adult_male: Optional[bool] = Field(
...
class StatisticsResponse(BaseModel):
    ...
    average_fare: Optional[float] = Field(
        None, description="Durchschnittlicher Ticketpreis"
    )
