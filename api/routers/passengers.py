...
def build_passenger_query(
    filters: Optional[PassengerFilter] = None,
...
async def get_passengers(
    ...
    survived: Optional[int] = Query(
        None, ge=0, le=1, description="Filter nach Überlebensstatus"
    ),
    pclass: Optional[int] = Query(
        None, ge=1, le=3, description="Filter nach Passagierklasse"
    ),
    ...
    adult_male: Optional[bool] = Query(
        None, description="Filter nach erwachsenen Männern"
    ),
    ...
    try:
        ...
        logger.info(
            f"Passagierliste abgerufen: {len(passengers)} von {total_count} Passagieren"
        )
...
async def get_passenger_statistics():
    ...
    try:
        ...
        survivors = (
            int(survival_result["survivors"]) if survival_result["survivors"] else 0
        )
        ...
        fare_query = (
            "SELECT AVG(fare) as avg_fare FROM Observation WHERE fare IS NOT NULL"
        )
        fare_result = execute_query(fare_query)[0]
        average_fare = (
            float(fare_result["avg_fare"]) if fare_result["avg_fare"] else None
        )
        ...
        class_distribution = {
            row["class"]: int(row["count"]) for row in class_results if row["class"]
        }
        ...
        gender_distribution = {
            row["sex"]: int(row["count"]) for row in gender_results if row["sex"]
        }
                ...
                class_survival.append(
                    {
                        ...
                        "survival_rate": (
                            float(row["survival_rate"]) if row["survival_rate"] else 0.0
                        ),
                    }
                )
