...
class TestAPIEndpoints:
    ...
    async def test_get_passengers_class_filter(self):
        ...
        async with httpx.AsyncClient(timeout=TEST_TIMEOUT) as client:
            # Test erste Klasse
            response = await client.get(
                f"{API_BASE_URL}/api/v1/passengers?pclass=1&limit=5"
            )
            assert response.status_code == 200
            data = response.json()
...
class TestStatisticsEndpoint:
    ...
    async def test_survival_by_class(self):
        ...
        async with httpx.AsyncClient(timeout=TEST_TIMEOUT) as client:
            response = await client.get(
                f"{API_BASE_URL}/api/v1/passengers/survival-by-class"
            )
...
class TestDataConsistency:
    ...
    async def test_total_count_consistency(self):
        ...
        async with httpx.AsyncClient(timeout=TEST_TIMEOUT) as client:
            ...
            passengers_response = await client.get(
                f"{API_BASE_URL}/api/v1/passengers?limit=1000"
            )
